...
function generateFakeData(): Conversation[] {
  ...
  const conversation: Conversation = {
    id: 1,
    user: null, // i % 2 === 0 ? userInfo : { id: selectedAssistantRef.value.id, username: selectedAssistantRef.value.name, avatarUrl: selectedAssistantRef.value.avatar },
    isMine: false,
    messages: [],
    date: new Date(),
  };
...
conversations.value = generateFakeData();
...
<template>
  <NVirtualList
    ref="conversationsVirtualListInst" :style="{ maxHeight: `${maxHeight}px` }"
    :item-size="conversations.length" :items="conversations" class="chat-virtual-list" item-resizable
  >
